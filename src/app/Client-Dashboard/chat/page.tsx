...
const ChatPage: React.FC = () => {
  ...
  const sendMessage = async () => {
    ...
    if (socket && isConnected) {
      try {
        ...
        sendMessageViaAPI(messageContent, optimisticMessage);
      }
    } else {
      sendMessageViaAPI(messageContent, optimisticMessage);
    }
  ...
  const sendMessageViaAPI = async (messageContent: string, optimisticMessage: Message) => {
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', height: '100%' }}>
        {/* Chat List Sidebar */}
        <Box sx={{ width: { xs: '100%', md: '30%' }, minWidth: 300 }}>
          <Paper
            sx={{
              height: '100%',
              display: 'flex',
          ...
          >
            ...
            {/* Connection Status */}
            <Box sx={{ px: 2, py: 1 }}>
              <Chip size="small" label={isConnected ? 'Connected' : 'Disconnected'} color={isConnected ? 'success' : 'error'} variant="outlined" />
            </Box>
        ...
        {/* Chat Area */}
        <Box sx={{ flex: 1, display: { xs: selectedChat ? 'flex' : 'none', md: 'flex' } }}>
              ...
              <>
                ...
                <Box sx={{ p: 2, borderBottom: `1px solid ${theme.palette.divider}` }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      <Typography variant="h6">{getChatDisplayName(selectedChat)}</Typography>

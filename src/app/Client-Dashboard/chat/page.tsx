...
const ChatPage: React.FC = () => {
  ...
  const [participantProfiles, setParticipantProfiles] = useState<
    Record<string, { profileImage?: string }>
  >({});
  ...
  const handleNewMessage = (data: { chatId: string; message: Message }) => {
    setChats((prevChats) =>
      prevChats.map((chat) => {
        ...
        const exists = messagesWithoutTemp.some(
          (m) => m._id === data.message._id,
        );
        const nextMessages = exists
          ? messagesWithoutTemp.map((m) =>
        ...
        chat._id === data.chatId
          ? {
              ...
              messages: chat.messages.map((msg) =>
                !msg.read && msg.sender._id !== data.readBy
    ...
    setChats((prevChats) =>
      prevChats.map((chat) => ({
        ...
        participants: chat.participants.map((p) =>
          p._id === data.userId
            ? { ...p, isOnline: data.isOnline, lastSeenAt: data.lastSeenAt }
            : p,
        ),
      })),
    );
  ...
  useEffect(() => {
    ...
    socketInstance.on(
      "messages_read",
      (data: { chatId: string; readBy: string; readAt: string }) => {
    ...
    socketInstance.on(
      ...
      (data: { chatId: string; userId: string; userName: string }) => {
        if (
          data.chatId === selectedChatIdRef.current &&
    ...
    try {
      ...
      if (data.success) {
        ...
        setChats(() => {
          ...
          if (currentSelected) {
            const updatedSelected = newChats.find(
              (c) => c._id === currentSelected._id,
            );
            if (updatedSelected) {
  ...
  const fetchAvailableUsers = async () => {
    try {
      ...
      if (data.success) {
        setAvailableUsers(
          (data.data as User[]).filter((u) => u._id !== getCurrentUserId()),
        );
      }
    } catch {
      // ignore
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success && data.data) {
              ...
              const profileImage =
                data.data.profileImage || data.data.profile?.profileImage;
              if (profileImage) {
      ...
      prev.map((chat) =>
        chat._id === selectedChat._id
          ? {
              ...
              lastMessage: optimisticMessage.timestamp,
            }
          : chat,
      ),
    );
  ...
  const sendMessageViaAPI = async (messageContent: string) => {
    try {
      ...
      if (data.success) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m._id.startsWith("temp_") && m.content === messageContent
              ? realMessage
              : m,
          ),
        );
        ...
        setMessages((prev) =>
          prev.filter(
            (m) => !(m._id.startsWith("temp_") && m.content === messageContent),
          ),
        );
        setChats((prev) =>
          prev.map((chat) =>
  ...
  const getChatAvatar = (chat: Chat) => {
    ...
    return profileImage ? (
      ...
      <Avatar>{other?.name?.charAt(0) || "U"}</Avatar>
    );
  };
  ...
  const typingNames =
    selectedChat && typingUsers.size > 0
      ? Array.from(typingUsers)
          .map(
            (id) =>
              selectedChat.participants.find((p) => p._id === id)?.name ||
              "User",
          )
          .join(", ")
      : "";
            ...
            {process.env.NODE_ENV === "development" && (
              ...
              >
                <Typography variant="caption" color="text.secondary">
                  Debug: {Object.keys(participantProfiles).length} profiles
                  cached
                </Typography>
              </Box>
            )}
                    ...
                    >
                      ...
                      >
                        ...
                        <Typography
                          ...
                          sx={{ fontSize: "0.8rem" }}
                        >
                          {typingNames} {typingUsers.size === 1 ? "is" : "are"}{" "}
                          typing...
                        </Typography>
                      </Box>
                    </Box>
      ...
      >
        ...
        <DialogContent>
          ...
          <FormControl fullWidth>
            <InputLabel>
              {newChatType === "direct" ? "Select User" : "Select Users"}
            </InputLabel>
            <Select
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              ...
              (newChatType === "group" && !groupName.trim())
            }
          >
            Create Chat
          </Button>
